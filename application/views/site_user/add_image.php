...
<div class="container">
  ...
  <div class="row">
    <div class="col-md-8">
      ...
      <?php echo form_open_multipart('add-image/'); ?>
      <div class="form-group">
        <label for="title">Title</label>
        <input type="text" name="title" class="form-control" id="title" aria-describedby="title_err" placeholder="Title" value="<?php echo set_value('title'); ?>">
        <small id="title_err" class="form-text text-danger"><?php echo form_error('title'); ?></small> </div>
      <div class="form-group">
        <label for="description">Description</label>
        <textarea name="description" class="form-control" id="description" placeholder="Description" aria-describedby="description_err"><?php echo set_value('description'); ?></textarea>
        ...
        <?php if ($categories) { ?>
          <select name="category_id" id="category_id" class="form-control" placeholder="Category" aria-describedby="category_id_err">
            ...
            <option value="<?php echo $category->id;?>" <?php echo set_select('category_id', $category->id); ?> ><?php echo $category->category;?></option>
            <?php } ?>
          </select>
        <?php } ?>
